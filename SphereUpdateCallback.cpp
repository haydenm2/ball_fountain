...
void SphereUpdateCallback::operator()(osg::Node* node, osg::NodeVisitor* visitingNode)
{
    ...
    osg::Vec3f positionOfBall(physics->get_ball_ptr(nodeNumber-2)->position[0], physics->get_ball_ptr(nodeNumber-2)->position[1], physics->get_ball_ptr(nodeNumber-2)->position[2]);
    ...
    ballShapeDrawable->setColor(osgwidgetutils::hue_to_osg_rgba_decimal(physics->get_ball_ptr(nodeNumber-2)->color));
    osg::Sphere *ball = new osg::Sphere(osg::Vec3(0.f, 0.f, 0.f), physics->get_ball_ptr(nodeNumber-2)->radius);
